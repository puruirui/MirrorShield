...
def construct_evaluation_prompt_gpt4o(pred_path, ref_path, outpath, genfirst, limit=None, defense_type=None):
    try:
...
if __name__ == '__main__':
    ...
    if model_path == 'gpt-4o':
        save_path = f'./vicuna/gen_results/gpt-4o{"" if not defense_type else "_" + defense_type}_output.jsonl'
        eval_gpt(outpath, save_path, model_name='gpt-4o', max_tokens=1024, top_p=0.1, temperature=0., limit=100, system_prompt='You are a helpful assistant.')
        process_chatgpt_gen(save_path, save_path[:-1])

    if model_path == 'gpt-4o':
        save_path = f'./vicuna/gen_results/gpt-4o{"" if not defense_type else "_" + defense_type}_output_withsystem_max1024.jsonl'
        eval_gpt(outpath, save_path, model_name='gpt-4o', max_tokens=1024, top_p=0.1, temperature=0., limit=100, system_prompt='You are a helpful assistant.')
        process_chatgpt_gen(save_path, save_path[:-1])
    ...
    outpath = f'./vicuna/eva_prompts/gpt4o_{model_name}{"" if not defense_type else "_" + defense_type}_davinci_randomorder.json'
    construct_evaluation_prompt_gpt4o(path1, path2, outpath, genfirst=genfirst, limit=100, defense_type=defense_type)
    ...
    if eva_name == 'chatgpt':
        ...
        save_path = f'./vicuna/eva_results/gpt4o_{model_name}{"" if not defense_type else "_" + defense_type}_davinci_gpt4_randomorder.jsonl'
        eval_gpt(file_path, save_path, model_name='gpt-4o', max_tokens=1024, top_p=0.1, temperature=0.)
